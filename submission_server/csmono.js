...
exports.judge = function (submission_id, languageData, problemData, time_limit, source_path, original_filename, test_cases) {
	console.log('-----------------CSMONO JUDGE-----------------');

	// Append .c to submission type...
	try {
		var stdout = execSync('mv ' + source_path + ' ' + source_path + '.cs', { encoding: 'utf8', timeout: 5000 } );
	} catch (err) {
		console.log('ERR moving file to add cs extension');
	...
	var executable_path = source_path + '.exe',
		cmd = 'mcs ' + source_path + '.cs -pkg:dotnet';
	try {
		execSync(cmd, { timeout: 5000, encoding: 'utf8' });
	} catch (error) {
		console.log('cpp98.js: ERR in building code: ' + error);
	...
	for (var i = 0; i < test_cases.length; i++) {
		...
		try {
			...
			try {
				...
				console.log('cpp98.js Error running comparison program: ' + error + ' (test case ' + i + ')');
				try {
					execSync('rm ' + out_file, { encoding: 'utf8', timeout: 5000 });
				} catch (err) {}
			...
			if (err.signal === 'SIGTERM') {
				...
				console.log('cpp98.js Error (test case ' + i + ') in executing command ' + cmd + ': ' + err);
