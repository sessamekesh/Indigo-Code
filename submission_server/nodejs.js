...
exports.judge = function (submission_id, languageData, problemData, time_limit, source_path, original_filename, test_cases) {
	console.log('-----------------NODEJS JUDGE-----------------');

	// Append .js to submission type...
	try {
		var stdout = execSync('mv ' + source_path + ' ' + source_path + '.js', { encoding: 'utf8', timeout: 5000 } );
	} catch (err) {
		console.log('ERR moving file to add js extension');
		console.log('--Source Path: ' + source_path);
		console.log('--New Path: ' + source_path + '.js');
	...
	for (var i = 0; i < test_cases.length; i++) {
		var out_file = sandbox_dir + '/test_result_p' + problemData.id + '_tc' + test_cases[i].id + '_sb' + submission_id,
			cmd = 'node ' + source_path + '.js < ' + sandbox_dir + '/tc' + test_cases[i].id + '.in > ' + out_file;
		try {
			...
			try {
				...
				console.log('nodejs.js Error running comparison program: ' + error + ' (test case ' + i + ')');
				try {
					execSync('rm ' + out_file, { encoding: 'utf8', timeout: 5000 });
				} catch (err) {}
			...
			if (err.signal === 'SIGTERM') {
				...
				console.log('nodejs.js Error (test case ' + i + ') in executing command ' + cmd + ': ' + err);
