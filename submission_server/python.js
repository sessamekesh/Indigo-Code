...
exports.judge = function (submission_id, languageData, problemData, time_limit, source_path, original_filename, test_cases) {
	...
	try {
		var stdout = execSync('mv ' + source_path + ' ' + source_path + '.py', { encoding: 'utf8', timeout: 5000 } );
	} catch (err) {
	...
	for (var i = 0; i < test_cases.length; i++) {
		var out_file = sandbox_dir + '/test_result_p' + problemData.id + '_tc' + test_cases[i].id + '_sb' + submission_id,
			cmd = 'python ' + source_path + '.py < ' + sandbox_dir + '/tc' + test_cases[i].id + '.in > ' + out_file;
		try {
			...
			try {
				...
				console.log('python.js: Error running comparison program: ' + error + ' (test case ' + i + ')');
				try {
					execSync('rm ' + out_file, { encoding: 'utf8', timeout: 5000 });
				} catch (err) {}
			...
			if (err.signal === 'SIGTERM') {
				...
				console.log('python.js: Error (test case ' + i + ') in executing command ' + cmd + ': ' + err);
