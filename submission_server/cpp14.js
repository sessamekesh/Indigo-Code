...
exports.judge = function (submission_id, languageData, problemData, time_limit, source_path, original_filename, test_cases) {
	console.log('-----------------CPP14 JUDGE-----------------');

	// Append .c to submission type...
	try {
		var stdout = execSync('mv ' + source_path + ' ' + source_path + '.cpp', { encoding: 'utf8', timeout: 5000 } );
	} catch (err) {
		console.log('ERR moving file to add cpp extension');
	...
	var executable_path = sandbox_dir + '/' + submission_id + '.exe',
		cmd = 'g++ -std=c++14 ' + source_path + '.cpp -o ' + executable_path;
	try {
		execSync(cmd, { timeout: 5000, encoding: 'utf8' });
	} catch (error) {
		console.log('cpp14.js: ERR in building code: ' + error);
	...
	for (var i = 0; i < test_cases.length; i++) {
		...
		try {
			...
			try {
				...
				console.log('cpp14.js Error running comparison program: ' + error + ' (test case ' + i + ')');
				try {
					execSync('rm ' + out_file, { encoding: 'utf8', timeout: 5000 });
				} catch (err) {}
			...
			if (err.signal === 'SIGTERM') {
				...
				console.log('cpp14.js Error (test case ' + i + ') in executing command ' + cmd + ': ' + err);
