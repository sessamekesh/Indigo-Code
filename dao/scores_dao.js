...
exports.updateScore = function(teamID, competitionID, incorrect_submission_time_penalty) {
	...
	getConnection().query(
		'SELECT DISTINCT problem_id FROM Submission '
			...
			+ 'WHERE result = \'AC\' AND team_id = ? AND Problem.competition_id = ?;',
	...
	function getSubmissionTimePenalty(problems_solved) {
		...
		getConnection().query(
			'SELECT submission_time, Competition.start_date AS start_time FROM Submission '
				...
				+ 'WHERE result=\'AC\' '
				+ 'AND team_id = ? AND Problem.competition_id = ? '
		...
		if (i >= problems_solved.length) {
			...
			getConnection().query(
				'SELECT COUNT(*) AS incorrect_count FROM Submission '
					...
					+ 'AND team_id = ? AND problem_id = ? '
					+ 'AND submission_time < (SELECT submission_time FROM '
						+ 'Submission WHERE result=\'AC\' AND team_id = ? '
						+ 'AND problem_id = ? ORDER BY submission_time ASC LIMIT 1);',
				[teamID, problems_solved[i].problem_id, teamID, problems_solved[i].problem_id],
				function (err, res) {
					if (err) {
	...
	function updateMahScore() {
		...
		if (score === undefined || time_penalty === undefined) {
			...
			getConnection().query(
				'INSERT INTO Score(team_id, competition_id, score, time_penalty) VALUES(?, ?, ?, ?) '
				+ 'ON DUPLICATE KEY UPDATE score=VALUES(score), time_penalty=VALUES(time_penalty);',
				[teamID, competitionID, score, time_penalty],
				function (err, res) {
					if (err) {
...
exports.getScoreboardData = function(compData, start, finish, callback) {
	...
	var query_text = 'SELECT Team.name AS team_name, Team.tagline AS tagline, '
	...
	problem_dao.getProblemsInCompetition(compData.id, function (res, err) {
		if (err) {
			...
			for (var i = 0; i < res.length; i++) {
				query_text += ', IF((SELECT COUNT(*) FROM Submission WHERE problem_id = ? AND team_id = Team.id AND result = \'AC\') > 0, \'SOLVED\', '
						+ 'IF((SELECT COUNT(*) FROM Submission WHERE problem_id = ? AND team_id = Team.id AND result <> \'AC\') < 0, \'ATTEMPTING\', \'EMPTY\')) AS '
						+ '? ';
	...
	function finishGeneratingAndPerformQuery() {
		query_text += 'FROM Score LEFT JOIN Team ON Score.team_id = Team.id '
			+ 'WHERE Score.competition_id = ? ORDER BY score DESC, time_penalty ASC LIMIT ?, ?;';
